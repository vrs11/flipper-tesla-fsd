...
static void process_frame(CanBusId bus, const CanFrame &frame) {
    ...
    if (frame.id == CAN_ID_STALK_ACTION) {
        state_enter();
        fsd_handle_stalk_action(&g_state, &frame, millis());
        state_exit();
...
void setup() {
    ...
    Serial.printf("[CFG] Test TX route: wheel 0x3C2 -> %s, stalk 0x340 -> %s, set-speed 0x2B9 -> %s\n",
